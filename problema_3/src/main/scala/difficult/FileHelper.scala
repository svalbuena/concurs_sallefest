...
class FileHelper(fileId: String) {
  ...
  def readFile(): List[Test] = {
    ...
    for (_ <- 0 until numOfTests) {
      ...
      val numOfWaiters: BigInt = firstLineFields(0).toInt
      val position: BigInt = firstLineFields(1).toInt

      var waiters: Array[BigInt] = secondLineFields.map(_.toInt).map(i => BigInt.int2bigInt(i))
  ...
  def writeFile(solutions: List[BigInt]): Unit = {
