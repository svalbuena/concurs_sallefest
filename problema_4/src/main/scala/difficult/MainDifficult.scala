...
object MainDifficult {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for ((test, i) <- tests.zipWithIndex.slice(0, 1)) {
  ...
  private def exec(programmers: Array[Int], teams: List[List[Int]], currentIndex: Int, currentTeam: List[Int]): Unit = {
    if (currentIndex == programmers.length) {
      if (teams.length > bestNumOfTeams){
        bestNumOfTeams = teams.length
        println("Best -> " + bestNumOfTeams)
      }
      ...
      if (currentTeam.nonEmpty && isNextTeamViable(teams, programmers(currentIndex)  :: currentTeam) && canIAddAProgrammer(programmers, teams, currentIndex)) {
  ...
  private def canIAddAProgrammer(programmers: Array[Int], teams: List[List[Int]], currentIndex: Int): Boolean = {
    val maxPossibleTeams = teams.length + (programmers.length - currentIndex - 1)

    maxPossibleTeams >= bestNumOfTeams
  }
}